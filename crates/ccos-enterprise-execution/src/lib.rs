...
mod tests {
    ...
    fn journal(tag: &str) -> (PathBuf, ExecutionJournal) {
        let dir = scratch(tag);
        let report = ExecutionJournal::open(dir.join("execution.jsonl"), "tenant-acme/session-1")
            .expect("open");
    ...
    fn append_reopen_and_verify_from_genesis() {
        ...
        drop(journal);
        let reopened =
            ExecutionJournal::open(dir.join("execution.jsonl"), "tenant-acme/session-1").unwrap();
    ...
    fn tampering_breaks_the_hash_chain() {
        ...
        let error = ExecutionJournal::open(&path, "tenant-acme/session-1")
            .err()
    ...
    fn incomplete_unterminated_tail_is_discarded() {
        ...
        assert!(
            matches!(reopened.tail_repair, TailRepair::DiscardedPartialTail { bytes } if bytes > 0)
        );
    ...
    fn recovery_distinguishes_not_started_unknown_and_completed() {
        ...
        journal
            .append(ExecutionEvent::ToolFinished {
                call_id: "call-1".into(),
                success: true,
                output_sha256: "output-hash".into(),
            })
            .unwrap();
        assert_eq!(
            journal.recover_tools().unwrap()[0].disposition,
    ...
    fn append_error_poisons_instance_until_reopen_repairs_tail() {
        ...
        assert!(
            matches!(report.tail_repair, TailRepair::DiscardedPartialTail { bytes } if bytes > 0)
        );
    ...
    fn a_journal_cannot_be_rebound_to_another_stream() {
        ...
        journal.append(requested("call-1", "cargo-test")).unwrap();
        drop(journal);
        let error = ExecutionJournal::open(dir.join("execution.jsonl"), "tenant-globex/session-1")
            .err()
            .expect("stream rebinding must fail");
        assert!(matches!(error, JournalError::Integrity(_)), "{error}");
        let _ = std::fs::remove_dir_all(dir);
    }
}
