...
impl DispatchExecution {
    pub fn new(turn_id: impl Into<String>, step_id: impl Into<String>, call_id: impl Into<String>) -> Self {
        Self { turn_id: turn_id.into(), step_id: step_id.into(), call_id: call_id.into() }
    }
    ...
    fn validate(&self) -> Result<(), ExecutionBackendError> {
        for (kind, value) in [
            ...
            if value.is_empty() {
                return Err(ExecutionBackendError::InvalidExecutionId { kind, detail: "must not be empty".into() });
            }
            if value.len() > 256 {
                return Err(ExecutionBackendError::InvalidExecutionId { kind, detail: "must not exceed 256 bytes".into() });
            }
            if value.chars().any(char::is_control) {
                return Err(ExecutionBackendError::InvalidExecutionId { kind, detail: "must not contain control characters".into() });
            }
        }
        Ok(())
    }
...
pub enum ExecutionBackendError {
    ...
    InvalidExecutionId { kind: &'static str, detail: String },
    Journal(JournalError),
    Serialize(serde_json::Error),
    Backend(String),
    OutcomeNotDurable { backend_succeeded: bool, journal: JournalError },
...
impl From<JournalError> for ExecutionBackendError { fn from(value: JournalError) -> Self { Self::Journal(value) } }
impl From<serde_json::Error> for ExecutionBackendError { fn from(value: serde_json::Error) -> Self { Self::Serialize(value) } }
...
impl<B> ExecutionBackend<B> {
    ...
    pub fn with_capacity(inner: B, root: impl AsRef<Path>, capacity: usize) -> Self {
        Self { inner, root: root.as_ref().to_path_buf(), capacity: capacity.max(1), journals: BTreeMap::new(), lru: VecDeque::new() }
    }

    pub fn inner(&self) -> &B { &self.inner }
    pub fn inner_mut(&mut self) -> &mut B { &mut self.inner }
    pub fn into_inner(self) -> B { self.inner }
    pub fn live_journal_count(&self) -> usize { self.journals.len() }
    ...
    fn touch(&mut self, tenant: &str) {
        if let Some(index) = self.lru.iter().position(|item| item == tenant) { self.lru.remove(index); }
        self.lru.push_back(tenant.to_string());
    }

    fn journal_for(&mut self, tenant: &str) -> Result<&mut ExecutionJournal, ExecutionBackendError> {
        let path = self.journal_path_for(tenant)?;
        if !self.journals.contains_key(tenant) {
            while self.journals.len() >= self.capacity {
                if let Some(victim) = self.lru.pop_front() { self.journals.remove(&victim); } else { break; }
            }
            let report = ExecutionJournal::open(path, format!("tenant/{tenant}/mcp"))?;
            self.journals.insert(tenant.to_string(), report.journal);
        }
        self.touch(tenant);
        Ok(self.journals.get_mut(tenant).expect("journal was inserted or already existed"))
    }

    pub fn recover_tools(&mut self, tenant: &str) -> Result<Vec<ToolRecovery>, ExecutionBackendError> {
    ...
    pub fn reconcile_finished(
        ...
        let call = recovered.iter().find(|item| item.call_id == call_id).ok_or_else(|| {
            ExecutionBackendError::RecoveryMismatch(format!("call {call_id:?} is absent from durable execution journal"))
        })?;
    ...
    pub fn ensure_no_unknown_outcomes(&mut self, tenant: &str) -> Result<(), ExecutionBackendError> {
        let unknown: Vec<String> = self.recover_tools(tenant)?
            .into_iter()
            .filter(|call| call.disposition == ToolRecoveryDisposition::OutcomeUnknown)
            .map(|call| call.call_id)
            .collect();
        if unknown.is_empty() { Ok(()) } else {
            Err(ExecutionBackendError::RecoveryMismatch(format!("unresolved outcome-unknown calls remain: {unknown:?}")))
        }
    }
    ...
    where B: Backend {
        ...
        let input_sha256 = sha256_hex(&serde_json::to_vec(arguments)?);
        {
            ...
            journal.append(ExecutionEvent::ToolStarted { call_id: execution.call_id.clone() })?;
        }
        ...
        let finish = self.journal_for(tenant)?.append(ExecutionEvent::ToolFinished {
            call_id: execution.call_id.clone(), success, output_sha256: output_hash,
        });
        if let Err(journal) = finish {
            return Err(ExecutionBackendError::OutcomeNotDurable { backend_succeeded: success, journal });
        }
...
impl<B: Backend> Backend for ExecutionBackend<B> {
    fn dispatch(&mut self, tenant: &str, core_tool: &str, arguments: &Value) -> Result<Value, String> {
        let sequence = self.journal_for(tenant).map_err(|error| error.to_string())?.len();
        let execution = DispatchExecution::new(
            format!("mcp-turn-{sequence}"), format!("mcp-step-{sequence}"), format!("mcp-call-{sequence}"),
        );
        self.dispatch_with_context(tenant, &execution, core_tool, arguments).map_err(|error| error.to_string())
    }
}
...
pub fn failed_output_sha256(detail: &str) -> String { sha256_hex(detail.as_bytes()) }

fn validate_tenant(tenant: &str) -> Result<(), ExecutionBackendError> {
    if is_canonical_identifier(tenant) { Ok(()) } else { Err(ExecutionBackendError::UnsafeTenantId(tenant.chars().take(64).collect())) }
}

fn validate_execution_value(kind: &'static str, value: &str) -> Result<(), ExecutionBackendError> {
    if value.is_empty() { return Err(ExecutionBackendError::InvalidExecutionId { kind, detail: "must not be empty".into() }); }
    if value.len() > 256 { return Err(ExecutionBackendError::InvalidExecutionId { kind, detail: "must not exceed 256 bytes".into() }); }
    if value.chars().any(char::is_control) { return Err(ExecutionBackendError::InvalidExecutionId { kind, detail: "must not contain control characters".into() }); }
    Ok(())
}
...
fn sha256_hex(input: &[u8]) -> String {
    ...
    for byte in digest { use std::fmt::Write as _; let _ = write!(output, "{byte:02x}"); }
    output
}
...
mod tests {
    ...
    impl Backend for BackendOk {
        fn dispatch(&mut self, _tenant: &str, _tool: &str, _args: &Value) -> Result<Value, String> { Ok(json!({"ok": true})) }
    }

    #[test]
    fn explicit_attempt_is_completed_and_reconciliation_is_idempotent() {
        let root = std::env::temp_dir().join(format!("ccos-mcp-exec-backend-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        let mut backend = ExecutionBackend::new(BackendOk, &root);
        let execution = DispatchExecution::new("turn-1", "step-2", "attempt-1");
        let value = backend.dispatch_with_context("acme", &execution, "recall", &json!({})).unwrap();
        let hash = successful_output_sha256(&value).unwrap();
        backend.reconcile_finished("acme", "attempt-1", true, &hash).unwrap();
