...
fn asset() -> MemoryAssetId {
    MemoryAssetId::new("evidence").unwrap()
}
    ...
    graph
        .register(
            MemoryAssetDescriptor::new(
                ...
                MemoryLineage::root([MemoryEvidenceRef::new("audit:evidence").unwrap()]).unwrap(),
            )
            .unwrap(),
        )
        .unwrap();
...
fn recall(
    ...
    let (assembly, attestations) = assemble_stored_governed_context(
        provider,
        store,
        expected,
...
fn admitted_request_tenant_is_checked_before_provider_access() {
    ...
    assert!(matches!(
        recall(&provider, &store, &TenantId::validated("other").unwrap()),
        Err(ServedContextError::Store(_))
    ));
...
fn poisoned_governance_owner_never_calls_the_provider() {
    ...
    let provider = Provider::default();
    assert!(matches!(
        recall(&provider, &store, &tenant()),
        Err(ServedContextError::Store(
