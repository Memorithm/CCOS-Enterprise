...
use crate::{MemoryAssetDescriptor, MemoryAssetId};
...
/// Opaque content digest supplied by the storage/export layer.
///
/// The memory contract deliberately does not choose a hashing algorithm. A
/// serializer or storage adapter must verify that the bytes it transports match
/// this digest before importing the manifest.
...
/// Opaque provider identifier and provider-local immutable reference.
...
/// One portable manifest entry for a governed memory asset.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl MemoryBundleEntry {
    pub fn new(
        ...
        if payload_bytes == 0 {
            return Err(MemoryBundleError::EmptyPayload);
        }
        Ok(Self {
            descriptor,
            content_digest,
...
/// Versioned, tenant-bound manifest for portable governed memory assets.
///
/// V1 is intentionally a manifest, not a wire encoding. It preserves the CCOS
/// identity, scope, stratum and lineage descriptors while leaving byte encoding,
/// encryption, signing and digest verification to the surrounding backup/export
/// layer. Derived assets must have all governed parents present in the same
/// bundle so an import cannot silently sever lineage.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl fmt::Display for MemoryBundleError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::EmptyPayload => write!(f, "memory bundle payload size must be non-zero"),
...
mod tests {
    ...
    fn derived(value: &str, parent: &str) -> MemoryAssetDescriptor {
        MemoryAssetDescriptor::new(
            id(value),
            MemorySpace::Tenant,
    ...
    fn entry(descriptor: MemoryAssetDescriptor) -> MemoryBundleEntry {
    ...
    #[test]
    fn manifest_preserves_identity_and_lineage() {
        let manifest = MemoryBundleManifest::v1(
            TenantId("tenant-a".into()),
            [entry(root("mem:root")), entry(derived("mem:episode", "mem:root"))],
        )
        .unwrap();

        assert_eq!(manifest.version(), MemoryBundleVersion::V1);
        assert_eq!(manifest.entries().len(), 2);
        assert_eq!(manifest.entries()[1].descriptor().id.as_str(), "mem:episode");
        assert_eq!(
            manifest.entries()[1]
                .descriptor()
                .lineage
    ...
    fn duplicate_asset_ids_fail_closed() {
        assert_eq!(
            MemoryBundleManifest::v1(
                TenantId("tenant-a".into()),
                [entry(root("mem:a")), entry(root("mem:a"))],
            ),
    ...
    fn derived_asset_cannot_silently_drop_parent_from_bundle() {
        assert_eq!(
            MemoryBundleManifest::v1(
                TenantId("tenant-a".into()),
                [entry(derived("mem:episode", "mem:missing"))],
            ),
    ...
    #[test]
    fn manifest_metadata_is_non_empty_and_payload_is_bounded_by_declared_size() {
        assert_eq!(
        ...
        assert_eq!(
            ...
            Err(MemoryBundleError::EmptyPayload)
        );
    }
}
