...
///
/// These limits are enforcement bounds, not ranking hints. They make response
/// size and backend shortlist work visible at the CCOS contract boundary.
...
/// A recall request whose backend work and returned payload are explicitly bounded.
...
/// Failure surface for post-provider CCOS recall enforcement.
...
/// Extension layer that applies CCOS-side bounds and trust-boundary checks to
/// every `SemanticMemoryProvider` without changing provider implementations.
...
mod tests {
    ...
    fn provider_cannot_escape_the_admitted_loadout() {
        ...
        assert!(matches!(
            error,
            MemoryRecallBudgetError::ProviderReturnedUnauthorizedSpace(MemorySpace::Team(id))
                if id == "forbidden"
        ));
    }
