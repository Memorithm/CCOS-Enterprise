...
#   - contribution trailers (six pinned historic redundant self-trailers only
#     are normalized below; no new contribution trailer is permitted);
...
# Six immutable 2026-08 squash objects contain a redundant trailer for the
# already-authorized maintainer. Normalize only those exact historic objects and
# only that exact single line. No author, committer or attribution check is skipped;
# every new object (including a replay of the same message) remains prohibited.
legacy_self_trailer() {
  case "$1" in
    bb65db19ca65a2ffe7618ea93afae55c810ca487|1e916f2aeda42e24f8333c8d4b7e54cab2fb95be|b078339c6fa4420e80402f9166f791ce7db97876|21b7cc41defdba79199ef7ffa771f21bd9651837|a82a1ab8967b2c2f8f0d7c1c2e1e1a212183ead9|18a7e2af4434b88635abcac5f76646eb5f97cdd2)
      [ "$2" = 'Co-authored-by: MEMOPERF <memoperf@users.noreply.github.com>' ] ;;
    *) return 1 ;;
  esac
}

# Process-substitution failures do not propagate through the while loop. Resolve
# the complete history first and fail closed on an invalid/empty revision or an
# incomplete clone, instead of printing PASS without examining a commit.
if [ "$(git rev-parse --is-shallow-repository)" != false ]; then
  echo '::error::author policy requires complete history (fetch-depth: 0)'
  exit 1
fi
history=$(git log --format='%H%x09%an%x09%ae%x09%cn%x09%ce%x09%P' "$RANGE" --)
if [ -z "$history" ]; then
  echo '::error::author policy selected no commits'
  exit 1
fi
...
while IFS=$'\t' read -r h an ae cn ce parents; do
  ...
  trailers=$(printf '%s\n' "$message" | grep -Ei "$TRAILER_RE" || true)
  if [ -n "$trailers" ] && ! legacy_self_trailer "$h" "$trailers"; then
